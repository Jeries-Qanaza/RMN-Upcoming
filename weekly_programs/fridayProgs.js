...
const friday = (time) => {
    ...
    else if (time >= 6.45 && time < 7.15) { currShow = 18; }
    else if (time >= 7.15 && time < 7.45) { currShow = 19; }
    ...
    else if (time >= 9 && time < 10) { currShow = 24; }
    else if (time >= 10 && time < 10.30) { currShow = 25; }
    ...
    for (let i = currShow+1; i < fridayArr.length; i++) {
        ...
        showName.innerHTML = dataPrograms.find(x => x.id == fridayArr[i]).progName

        //show image
        const showImage = document.createElement("img")
        showImage.className = "showIm"
        showImage.src = dataPrograms.find(x => x.id == fridayArr[i]).progImg
